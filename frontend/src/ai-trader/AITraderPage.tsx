...
function PerformanceChart() {
  ...
  return <div className="performance-chart-wrap"><div className="chart-summary"><div><span>{t('aiTrader.currentValue')}</span><strong>{formatCurrency(traderSummary.currentValue)}</strong></div><div><b>+{formatCurrency(traderSummary.profitLoss)}</b><small>{t('aiTrader.allTime', { percent: traderSummary.returnPercent.toFixed(2) })}</small></div></div><svg aria-label={t('aiTrader.performanceChart')} className="performance-chart" role="img" viewBox={`0 0 ${width} ${height}`}><defs><linearGradient id="ai-trader-fill" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="#2f7bf0" stopOpacity=".2" /><stop offset="100%" stopColor="#2f7bf0" stopOpacity="0" /></linearGradient></defs>{[0, 1, 2, 3].map((tick) => { const y = padding.top + (tick / 3) * usableHeight; const value = max - (tick / 3) * (max - min); return <g key={tick}><line className="chart-grid-line" x1={padding.left} x2={width - padding.right} y1={y} y2={y} /><text className="chart-y-label" textAnchor="end" x={width - padding.right} y={y - 5}>{`$${Math.round(value / 1000)}K`}</text></g> })}<path className="chart-area" d={areaPath} fill="url(#ai-trader-fill)" /><path className="chart-line" d={linePath} />{activePoint && <line className="chart-crosshair" x1={activePoint.x} x2={activePoint.x} y1={padding.top} y2={height - padding.bottom} />}{points.map((point, index) => <g className="chart-point-group" key={`${point.x}-${point.value}`} onPointerEnter={() => setActiveIndex(index)} onPointerLeave={() => setActiveIndex(null)}><circle className="chart-point-hit" cx={point.x} cy={point.y} r="20" /><circle className="chart-point" cx={point.x} cy={point.y} r={index === points.length - 1 ? 4 : 2.5} /></g>)}{activePoint && activeIndex !== null && <g className="chart-tooltip" pointerEvents="none" transform={`translate(${tooltipX} ${tooltipY})`}><rect height="43" rx="6" width="118" /><text x="9" y="16">{t(performanceLabels[Math.min(performanceLabels.length - 1, Math.round(activeIndex / 2))])}</text><text className="chart-tooltip-value" x="9" y="33">{formatCurrency(activePoint.value)}</text></g>}{performanceLabels.map((label, index) => <text className="chart-x-label" key={label} textAnchor={index === 0 ? 'start' : index === performanceLabels.length - 1 ? 'end' : 'middle'} x={padding.left + (index / (performanceLabels.length - 1)) * usableWidth} y={height - 6}>{t(label)}</text>)}</svg></div>
}
...
function PerformanceStatsPanel() {
  const { t } = useTranslation()
  return <section className="panel stats-panel"><PanelHeading title={t('aiTrader.performanceStatistics')} subtitle={t('aiTrader.recentBotPerformance')} /><div className="stats-content"><div className="stat-list"><div><span>{t('aiTrader.totalReturn')}</span><strong className="positive">+12.85%</strong></div><div><span>{t('aiTrader.accuracy')}</span><strong>72.4%</strong></div><div><span>{t('aiTrader.winRate')}</span><strong>68.7%</strong></div><div><span>{t('aiTrader.averageWin')}</span><strong className="positive">+2.35%</strong></div><div><span>{t('aiTrader.totalTrades')}</span><strong>42</strong></div><div><span>{t('aiTrader.bestTrade')}</span><strong className="positive">+8.42%</strong></div></div><Sparkline /></div></section>
}

function ModelPanel({ detailed = false }: { detailed?: boolean }) {
  const { t } = useTranslation()
  return <section className={`panel model-panel ${detailed ? 'model-panel-detailed' : ''}`}><PanelHeading title={t('aiTrader.modelVersion')} subtitle={t('aiTrader.modelHealth')} /><div className="model-version-row"><span className="model-badge"><Icon name="sparkles" size={15} /></span><div><strong>{t('aiTrader.modelLabel')} {currentModel.version}</strong><small>{t('aiTrader.currentProductionModel')}</small></div><b className="current-badge">{t('aiTrader.current')}</b></div><div className="model-metrics"><div><span>{t('aiTrader.accuracy')}</span><strong>{currentModel.accuracy}</strong></div><div><span>{t('aiTrader.precision')}</span><strong>{currentModel.precision}</strong></div><div><span>{t('aiTrader.recall')}</span><strong>{currentModel.recall}</strong></div><div><span>{t('aiTrader.f1Score')}</span><strong>{currentModel.f1Score}</strong></div><div><span>{t('aiTrader.trainedOn')}</span><strong>{currentModel.trainedOnKey ? t(currentModel.trainedOnKey) : currentModel.trainedOn}</strong></div><div><span>{t('aiTrader.nextRetrain')}</span><strong>{t('aiTrader.nextRetrainValue')}</strong></div></div>{detailed && <div className="model-history">{['v3.2.1', 'v3.1.0', 'v3.0.2'].map((version, index) => <div key={version}><span>{version}</span><small>{index === 0 ? t('aiTrader.current') : t('aiTrader.archived')} · {index === 0 ? currentModel.accuracy : index === 1 ? '70.8%' : '68.5%'}</small><Icon name="chevron-right" size={14} /></div>)}</div>}<button className="outline-button" onClick={() => undefined} type="button">{t('aiTrader.viewModelDetails')} <Icon name="chevron-right" size={13} /></button></section>
}

function BacktestPanel({ detailed = false }: { detailed?: boolean }) {
  const { t } = useTranslation()
  return <section className={`panel backtest-panel ${detailed ? 'backtest-panel-detailed' : ''}`}><PanelHeading title={t('aiTrader.backtestingResults')} subtitle={t('aiTrader.historicalSimulation')} /><div className="backtest-header"><span>{t('aiTrader.lastSixMonths')}</span><Icon name="chevron-down" size={13} /></div><div className="backtest-content"><div className="backtest-metrics"><div><span>{t('aiTrader.totalReturn')}</span><strong className="positive">{backtestSummary.totalReturn}</strong></div><div><span>{t('aiTrader.winRate')}</span><strong>{backtestSummary.winRate}</strong></div><div><span>{t('aiTrader.maxDrawdown')}</span><strong className="negative">{backtestSummary.maxDrawdown}</strong></div><div><span>{t('aiTrader.sharpeRatio')}</span><strong>{backtestSummary.sharpeRatio}</strong></div><div><span>{t('aiTrader.totalTrades')}</span><strong>{backtestSummary.totalTrades}</strong></div><div><span>{t('aiTrader.profitFactor')}</span><strong>{backtestSummary.profitFactor}</strong></div></div><Sparkline tone="purple" /></div>{detailed && <div className="backtest-history"><div><span>{t('aiTrader.strategies.momentumMay')}</span><strong className="positive">+28.12%</strong></div><div><span>{t('aiTrader.strategies.balancedApr')}</span><strong className="positive">+19.64%</strong></div><div><span>{t('aiTrader.strategies.baselineMar')}</span><strong className="positive">+14.22%</strong></div></div>}<button className="outline-button" onClick={() => undefined} type="button">{t('aiTrader.viewFullBacktest')} <Icon name="chevron-right" size={13} /></button></section>
}
...
function Overview({ setActiveTab }: { setActiveTab: (tab: TabName) => void }) {
  const { t } = useTranslation()
  return <><section className="metric-grid"><MetricCard change="+$12,846.37" icon="wallet" label={t('aiTrader.initialCapital')} tone="blue" value={formatCurrency(traderSummary.initialCapital)} /><MetricCard change="+12.85%" icon="chart" label={t('aiTrader.currentValueMetric')} tone="green" value={formatCurrency(traderSummary.currentValue)} /><MetricCard change="+12.85%" icon="trending-up" label={t('aiTrader.totalProfitLoss')} tone="purple" value={formatSignedCurrency(traderSummary.profitLoss)} /><MetricCard change="+3.4%" icon="activity" label={t('aiTrader.winRate')} tone="orange" value={`${traderSummary.winRate}%`} /><MetricCard change="↓ 1.12%" icon="pie-chart" label={t('aiTrader.maxDrawdown')} tone="red" value={`${traderSummary.maxDrawdown.toFixed(2)}%`} /></section><section className="panel performance-panel"><PanelHeading title={t('dashboard.performanceTitle')} subtitle={t('aiTrader.portfolioValueOverTime')} action={t('aiTrader.viewDetails')} onAction={() => setActiveTab('Performance')} /><div className="range-tabs" role="tablist"><button className="selected" type="button">1M</button><button type="button">3M</button><button type="button">6M</button><button type="button">1Y</button><button type="button">ALL</button></div><PerformanceChart /></section><div className="overview-grid overview-grid-top"><section className="panel positions-panel"><PanelHeading title={t('aiTrader.openPositions', { count: 5 })} subtitle={t('aiTrader.currentBotHoldings')} action={t('common.viewAll')} onAction={() => setActiveTab('Positions')} /><PositionTable compact /><span className="panel-footnote">{t('aiTrader.totalUnrealizedPnl')} <strong className="positive">+{formatCurrency(27472.77)} (+24.36%)</strong></span></section><section className="panel decisions-panel"><PanelHeading title={t('aiTrader.currentDecisions')} subtitle={t('aiTrader.latestModelSignals')} action={t('common.viewAll')} onAction={() => setActiveTab('Decisions')} /><DecisionTable /></section><section className="panel rejected-panel"><PanelHeading title={t('aiTrader.rejectedDecisionsRisk')} subtitle={t('aiTrader.signalsBlockedBySafeguards')} action={t('common.viewAll')} onAction={() => setActiveTab('Decisions')} /><RejectedTable /></section></div><section className="panel trades-panel"><PanelHeading title={t('aiTrader.recentTrades')} subtitle={t('aiTrader.latestExecutedOrders')} action={t('common.viewAll')} onAction={() => setActiveTab('Trades')} /><TradesTable compact /></section><div className="overview-grid overview-grid-bottom"><PerformanceStatsPanel /><ModelPanel /><BacktestPanel /></div></>
}
...
function TabContent({ activeTab, setActiveTab }: { activeTab: TabName; setActiveTab: (tab: TabName) => void }) {
  ...
  return <div className="standalone-grid"><BacktestPanel detailed /><section className="panel standalone-panel backtest-notes"><PanelHeading title={t('aiTrader.backtestConfiguration')} subtitle={t('aiTrader.simulationSettings')} /><div className="config-list"><div><span>{t('aiTrader.startingCapital')}</span><strong>$100,000.00</strong></div><div><span>{t('aiTrader.strategy')}</span><strong>{t('aiTrader.strategies.momentumRisk')}</strong></div><div><span>{t('aiTrader.period')}</span><strong>{t('aiTrader.periodValue')}</strong></div><div><span>{t('aiTrader.tradingUniverse')}</span><strong>{t('aiTrader.strategies.usLargeCap')}</strong></div></div><button className="primary-button" onClick={() => undefined} type="button"><Icon name="play" size={14} /> {t('aiTrader.runNewBacktest')}</button></section></div>
}
