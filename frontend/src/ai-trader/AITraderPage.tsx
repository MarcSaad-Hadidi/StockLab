...
function RejectedTable() {
  ...
  return <div className="table-scroll"><table className="data-table rejected-table"><thead><tr><th>{t('aiTrader.table.asset')}</th><th>{t('aiTrader.table.action')}</th><th>{t('aiTrader.table.reason')}</th><th>{t('aiTrader.table.confidence')}</th><th>{t('aiTrader.table.time')}</th></tr></thead><tbody>{rejectedDecisions.map((decision) => <tr key={`${decision.symbol}-${decision.time}`}><td><strong>{decision.symbol}</strong></td><td><ActionBadge action={decision.action} /></td><td className="reason-cell">{t(reasonKeys[decision.reason as keyof typeof reasonKeys])}</td><td>{decision.confidence}%</td><td className="time-cell">{decision.time}</td></tr>)}</tbody></table></div>
}

function TradesTable({ compact = false }: { compact?: boolean }) {
  const { t } = useTranslation()
  const rows = compact ? recentTrades.slice(0, 4) : recentTrades
  return <div className="table-scroll"><table className="data-table trades-table"><thead><tr><th>{t('aiTrader.table.asset')}</th><th>{t('aiTrader.table.side')}</th><th>{t('aiTrader.table.entryExit')}</th><th>{t('aiTrader.table.price')}</th><th>{t('aiTrader.table.pnlDollar')}</th><th>{t('aiTrader.table.pnlPercent')}</th><th>{t('aiTrader.table.time')}</th></tr></thead><tbody>{rows.map((trade) => <tr key={`${trade.symbol}-${trade.time}`}><td><strong>{trade.symbol}</strong></td><td><ActionBadge action={trade.side} /></td><td>{t(`aiTrader.table.${trade.type.toLowerCase()}`)}</td><td>{formatCurrency(trade.price)}</td><td className={trade.pnl >= 0 ? 'positive' : 'negative'}>{formatSignedCurrency(trade.pnl)}</td><td className={trade.pnlPercent >= 0 ? 'positive' : 'negative'}>{formatSignedPercent(trade.pnlPercent)}</td><td className="time-cell">{trade.time}</td></tr>)}</tbody></table></div>
}
...
function ModelPanel({ detailed = false }: { detailed?: boolean }) {
  const { t } = useTranslation()
  return <section className={`panel model-panel ${detailed ? 'model-panel-detailed' : ''}`}><PanelHeading title={t('aiTrader.modelVersion')} subtitle={t('aiTrader.modelHealth')} /><div className="model-version-row"><span className="model-badge"><Icon name="sparkles" size={15} /></span><div><strong>Model {currentModel.version}</strong><small>{t('aiTrader.currentProductionModel')}</small></div><b className="current-badge">{t('aiTrader.current')}</b></div><div className="model-metrics"><div><span>{t('aiTrader.accuracy')}</span><strong>{currentModel.accuracy}</strong></div><div><span>{t('aiTrader.precision')}</span><strong>{currentModel.precision}</strong></div><div><span>{t('aiTrader.recall')}</span><strong>{currentModel.recall}</strong></div><div><span>{t('aiTrader.f1Score')}</span><strong>{currentModel.f1Score}</strong></div><div><span>{t('aiTrader.trainedOn')}</span><strong>{currentModel.trainedOn}</strong></div><div><span>{t('aiTrader.nextRetrain')}</span><strong>May 27, 2024</strong></div></div>{detailed && <div className="model-history">{['v3.2.1', 'v3.1.0', 'v3.0.2'].map((version, index) => <div key={version}><span>{version}</span><small>{index === 0 ? t('aiTrader.current') : t('aiTrader.archived')} · {index === 0 ? currentModel.accuracy : index === 1 ? '70.8%' : '68.5%'}</small><Icon name="chevron-right" size={14} /></div>)}</div>}<button className="outline-button" onClick={() => undefined} type="button">{t('aiTrader.viewModelDetails')} <Icon name="chevron-right" size={13} /></button></section>
}
...
function TabContent({ activeTab, setActiveTab }: { activeTab: TabName; setActiveTab: (tab: TabName) => void }) {
  ...
  return <div className="standalone-grid"><BacktestPanel detailed /><section className="panel standalone-panel backtest-notes"><PanelHeading title={t('aiTrader.backtestConfiguration')} subtitle={t('aiTrader.simulationSettings')} /><div className="config-list"><div><span>{t('aiTrader.startingCapital')}</span><strong>$100,000.00</strong></div><div><span>{t('aiTrader.strategy')}</span><strong>{t('aiTrader.strategies.momentumRisk')}</strong></div><div><span>{t('aiTrader.period')}</span><strong>Dec 2023 – May 2024</strong></div><div><span>{t('aiTrader.tradingUniverse')}</span><strong>{t('aiTrader.strategies.usLargeCap')}</strong></div></div><button className="primary-button" onClick={() => undefined} type="button"><Icon name="play" size={14} /> {t('aiTrader.runNewBacktest')}</button></section></div>
}
