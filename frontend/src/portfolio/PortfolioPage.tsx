...
function PerformanceChart({ range, onRangeChange }: { range: TimeRange; onRangeChange: (range: TimeRange) => void }) {
  ...
  const chartEndY: Record<TimeRange, number> = { '1D': 12, '1W': 22, '1M': 27, '3M': 46, '1Y': 23, YTD: 22, ALL: 30 }
  const endY = series.at(-1)?.y ?? chartEndY[range]
  ...
  return <section className="panel performance-panel"><div className="panel-header"><div><h2>Portfolio Performance <Icon name="info" /></h2></div><div className="range-switch" role="group" aria-label="Performance range">{(Object.keys(chartPoints) as TimeRange[]).map((option) => <button key={option} type="button" className={range === option ? 'active' : ''} aria-pressed={range === option} onClick={() => { setHoveredPoint(null); onRangeChange(option) }}>{option}</button>)}</div></div><div className="chart-wrap"><svg className="performance-chart" viewBox="0 0 720 230" preserveAspectRatio="none" aria-label="Portfolio performance chart. Move across the line to inspect values." role="img" onPointerMove={handlePointerMove} onPointerDown={handlePointerMove} onPointerLeave={() => setHoveredPoint(null)} onPointerCancel={() => setHoveredPoint(null)}><defs><linearGradient id="performance-fill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#7082ff" stopOpacity=".26" /><stop offset="1" stopColor="#7082ff" stopOpacity="0" /></linearGradient></defs><g className="chart-grid"><line x1="0" y1="20" x2="700" y2="20" /><line x1="0" y1="65" x2="700" y2="65" /><line x1="0" y1="110" x2="700" y2="110" /><line x1="0" y1="155" x2="700" y2="155" /><line x1="0" y1="200" x2="700" y2="200" /></g><text x="5" y="19">$140K</text><text x="5" y="64">$120K</text><text x="5" y="109">$100K</text><text x="5" y="154">$80K</text><text x="5" y="199">$60K</text><g key={range} className="chart-visual"><polygon points={`${points} 680,205 0,205`} fill="url(#performance-fill)" /><polyline points={points} pathLength="1" fill="none" stroke="#5870ff" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" /><circle cx="680" cy={endY} r="4" fill="#fff" stroke="#5870ff" strokeWidth="2" /><g className="chart-callout"><rect x="642" y={endY - 16} width="57" height="23" rx="5" /><text x="650" y={endY - 1}>$128.55K</text></g></g>{hoveredPoint && <g className="chart-hover" pointerEvents="none"><line x1={hoveredPoint.x} y1="20" x2={hoveredPoint.x} y2="205" /><circle className="hover-dot" cx={hoveredPoint.x} cy={hoveredPoint.y} r="4.5" /><g className="hover-tooltip" transform={`translate(${tooltipX} ${tooltipY})`}><rect width="90" height="45" rx="6" /><text className="hover-date" x="9" y="17">{hoveredPoint.date}</text><text className="hover-value" x="9" y="34">{hoveredPoint.value}</text></g></g>}</svg><div className="chart-x-axis"><span>Feb 24</span><span>Mar 10</span><span>Mar 24</span><span>Apr 7</span><span>Apr 21</span><span>May 5</span><span>May 19</span></div>{hoveredPoint && <p className="chart-hover-readout" aria-live="polite">{hoveredPoint.date} · {hoveredPoint.value}</p>}</div><div className="performance-stats"><div><span>YTD Return</span><strong>↗ 6.47%</strong></div><div><span>1Y Return</span><strong>↗ 18.92%</strong></div><div><span>Best Day</span><strong>↗ $2,341.22</strong></div><div className="negative"><span>Worst Day</span><strong>↘ -$1,124.36</strong></div></div></section>
}
