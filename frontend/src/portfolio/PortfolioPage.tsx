...
import { chartPoints, positions } from './portfolioData'
...
function PerformanceChart({ range, onRangeChange }: { range: TimeRange; onRangeChange: (range: TimeRange) => void }) {
  ...
  return <section className="panel performance-panel"><div className="panel-header"><div><h2>Portfolio Performance <Icon name="info" /></h2></div><div className="range-switch" role="group" aria-label="Performance range">{(Object.keys(chartPoints) as TimeRange[]).map((option) => <button key={option} type="button" className={range === option ? 'active' : ''} aria-pressed={range === option} onClick={() => onRangeChange(option)}>{option}</button>)}</div></div><div className="chart-wrap"><svg className="performance-chart" viewBox="0 0 720 230" preserveAspectRatio="none" aria-label="Portfolio performance chart" role="img"><defs><linearGradient id="performance-fill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#7082ff" stopOpacity=".26" /><stop offset="1" stopColor="#7082ff" stopOpacity="0" /></linearGradient></defs><g className="chart-grid"><line x1="0" y1="20" x2="700" y2="20" /><line x1="0" y1="65" x2="700" y2="65" /><line x1="0" y1="110" x2="700" y2="110" /><line x1="0" y1="155" x2="700" y2="155" /><line x1="0" y1="200" x2="700" y2="200" /></g><text x="5" y="19">$140K</text><text x="5" y="64">$120K</text><text x="5" y="109">$100K</text><text x="5" y="154">$80K</text><text x="5" y="199">$60K</text><polygon points={`${points} 680,205 0,205`} fill="url(#performance-fill)" /><polyline points={points} fill="none" stroke="#5870ff" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" /><circle cx="680" cy="20" r="4" fill="#fff" stroke="#5870ff" strokeWidth="2" /><g className="chart-callout"><rect x="642" y="4" width="57" height="23" rx="5" /><text x="650" y="19">$128.55K</text></g></svg><div className="chart-x-axis"><span>Feb 24</span><span>Mar 10</span><span>Mar 24</span><span>Apr 7</span><span>Apr 21</span><span>May 5</span><span>May 19</span></div></div><div className="performance-stats"><div><span>YTD Return</span><strong>↗ 6.47%</strong></div><div><span>1Y Return</span><strong>↗ 18.92%</strong></div><div><span>Best Day</span><strong>↗ $2,341.22</strong></div><div className="negative"><span>Worst Day</span><strong>↘ -$1,124.36</strong></div></div></section>
}
