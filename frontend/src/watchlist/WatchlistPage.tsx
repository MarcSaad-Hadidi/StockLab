    ...
    <div className="watchlist-page stocklab-layout">
      ...
      <main className="watchlist-main">
        ...
        <div className="watchlist-content">
          ...
          <section aria-label={t('watchlist.summaryLabel')} className="watchlist-summary"><div><span className="summary-icon summary-icon-blue"><Icon name="star" size={18} /></span><span><small>{t('watchlist.savedAssets')}</small><strong>{formatNumber(items.length, undefined, 0)}</strong></span></div><div><span className="summary-icon summary-icon-green"><Icon name="trending-up" size={18} /></span><span><small>{t('watchlist.gainersToday')}</small><strong>{formatNumber(items.filter((item) => item.tone === 'positive').length, undefined, 0)}</strong></span></div><div><span className="summary-icon summary-icon-purple"><Icon name="activity" size={18} /></span><span><small>{t('watchlist.averageMove')}</small><strong>{'—'}</strong></span></div><div className="summary-status">{t('businessData.unavailable')}</div></section>

          <section aria-labelledby="watchlist-title" className="panel watchlist-panel"><div className="panel-heading"><div><h2 id="watchlist-title">{t('watchlist.stocksWatching')}</h2><p>{t('businessData.backendPending')}</p></div><button className="panel-action" disabled title={t('businessData.unavailable')} type="button"><Icon name="activity" size={14} /> {t('watchlist.refreshPrices')}</button></div><div className="watchlist-controls"><label className="watchlist-filter"><Icon name="search" size={16} /><input aria-label={t('watchlist.filterStocks')} onChange={(event) => setQuery(event.target.value)} placeholder={t('watchlist.filterPlaceholder')} value={query} /></label><label className="sort-control"><span>{t('watchlist.sortBy')}</span><select aria-label={t('watchlist.sortLabel')} onChange={(event) => setSort(event.target.value as 'default' | 'price' | 'change')} value={sort}><option value="default">{t('watchlist.addedRecently')}</option><option value="price">{t('watchlist.priceHighLow')}</option><option value="change">{t('watchlist.dailyChange')}</option></select><Icon name="chevron-down" size={14} /></label></div><div className="watchlist-columns" aria-hidden="true"><span>{t('common.asset')}</span><span>{t('watchlist.currentPrice')}</span><span>{t('common.today')}</span><span>{t('common.actions')}</span></div><div className="watchlist-list">{filteredItems.length > 0 ? filteredItems.map((item) => <WatchlistRow item={item} key={item.symbol} onAlert={setAlertTarget} onDetails={item => window.location.assign(`${routeFor('market')}?symbol=${encodeURIComponent(item.symbol)}`)} onRemove={removeItem} />) : <div className="empty-state"><span><Icon name="search" size={19} /></span><strong>{t('businessData.watchlist')}</strong><p>{t('businessData.backendPending')}</p></div>}</div><div className="watchlist-footer"><span><i className="live-dot" /> {t('businessData.backendPending')}</span><strong>{t('watchlist.assetsShown', { shown: filteredItems.length, total: items.length })}</strong></div></section>
        </div>
      </main>
