...
function ProfileField({ label, value, editing, type = 'text', onChange, onEdit, readOnly = false }: { label: string; value: string; editing: boolean; type?: 'text' | 'email' | 'password'; onChange?: (value: string) => void; onEdit: () => void; readOnly?: boolean }) {
  return <div className="profile-info-row"><span className="profile-info-label">{label}</span>{editing && !readOnly ? <input aria-label={label} onChange={(event) => onChange?.(event.target.value)} required type={type} value={value} /> : <strong>{value}</strong>}<button aria-label={`Edit ${label}`} className="row-edit-button" onClick={onEdit} type="button"><Icon name="edit" size={14} /></button></div>
}
...
const navigation = [
  ...
  { label: 'Transactions', icon: 'activity' as IconName, href: '#transactions' },
